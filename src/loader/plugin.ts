...
import { getHash, useNumOnly, transformPX, getSlices, getOutput } from "../core/util";
    ...
    return {
      ...
      async Declaration(decl) {
        if (decl.prop === property) {
          ...
          const optionHash = getHash(`${direction}${slice.join(",")}`);
          ...
          let imgHeight;
          let oldCache =
            hasAlreadyOutput && getImageCache(fileHash, optionHash);
          const currentOption = {};
          let options = {};
          if (oldCache) {
            const {
            ...
            oldCacheOption[fileHash] = Object.assign(
              oldCacheOption[fileHash] || {},
              {
                options: Object.assign(
                  oldCacheOption[fileHash]?.options || {},
                  {
          ...
          const tasks = sliceArr.map((slice, ind) => {
            const itemName = getOutput(
              output,
              urlParse.name,
              ind,
              optionHash.substr(0, 5)
            );
            ...
            let prm;
            const hasFile =
              fs.pathExistsSync(resultPath) &&
              bgsResource.find((bg) => bg.ind === ind)?.filePath === resultPath;
            if (useCache && hasFile) {
              prm = Promise.resolve();
